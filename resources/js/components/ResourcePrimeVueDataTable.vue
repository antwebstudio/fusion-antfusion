        ...
        <antfusion-primevue-datatable v-bind="$props" :filters="filterValues" @update-metrics="updateMetrics">
            <template v-slot:bulkActions="{ parent, allowedBulkActions, selected }">
                <div class="ml-auto">
                    ...
                    <select name="bulk-actions" id="bulk-actions" class="field-select field-select--sm field-select--bordered" v-model="action" @change="initForm(parent, allowedBulkActions); showBulkActionConfirmation = true">
                ...
                <portal to="modals">
                    ...
                    <ui-modal v-if="action !== null" name="confirm-bulk-action" :title="'Confirm Bulk ' + allowedBulkActions[action].name" v-model="showBulkActionConfirmation">
                        <div v-if="allowedBulkActions[action].params && allowedBulkActions[action].params.fields.length">
                            <span v-if="form">
                                <component v-model="form[field.handle]" v-for="field, index in allowedBulkActions[action].params.fields" :key="field.handle" :is="field.component" v-bind="field"
                                    :parent="allowedBulkActions[action]"
                                    :record="record"
                        ...
                        <p v-else >Are you sure you want to perform this action against <b>{{ selected.length }}</b> record{{selected.length > 1 ? 's' : '' }}?</p>


                        <template v-slot:footer>
                            <ui-button @click.prevent="confirmBulkAction(parent, allowedBulkActions)" :loading="working" class="ml-3" variant="primary">Confirm</ui-button>
                            <ui-button @click.prevent="cancelBulkAction(parent, allowedBulkActions)" v-if="! working" variant="secondary">Cancel</ui-button>
                        </template>
                    </ui-modal>
                </portal>
            </template>
...
export default {
    ...
    methods: {
        ...
        cancelBulkAction(dataTable, allowedBulkActions) {
            this.action = null
            this.showBulkActionConfirmation = false
        },
        confirmBulkAction(dataTable, allowedBulkActions) {
            let action = allowedBulkActions[this.action]
            let vm = this
        ...
        initForm(dataTable, allowedBulkActions) {
            let action = allowedBulkActions[this.action]
